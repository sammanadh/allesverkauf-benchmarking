...
async function runUserJourney(url, appName, runNumber) {
  ...
  await page.evaluateOnNewDocument(() => {
    ...
    document.addEventListener('click', () => window.interactionCount++, true);
    document.addEventListener('input', () => window.interactionCount++, true);
  });
  ...
  try {
    ...
    const interactionCount = await page.evaluate(() => window.interactionCount || 0);
...
async function main() {
  console.log('🎭 User Journey Interaction Counter\n');
  ...
  for (const [appName, appUrl] of Object.entries(APPS)) {
    ...
    // Calculate median
    const counts = runs.map(r => r.totalInteractions).sort((a, b) => a - b);
    const medianCount = counts[Math.floor(counts.length / 2)];

    allResults[appName] = {
      medianInteractionCount: medianCount,
      runs: runs
    };

    console.log(`\n  📊 Median Interactions: ${medianCount}`);
  }
  ...
  // Print comparison
  console.log('\n\n📊 Results:\n');
  console.log('| Variant     | Interactions |');
  console.log('|-------------|--------------|');

  for (const [appName, results] of Object.entries(allResults)) {
    console.log(`| ${appName.padEnd(11)} | ${String(results.medianInteractionCount).padStart(12)} |`);
  }
